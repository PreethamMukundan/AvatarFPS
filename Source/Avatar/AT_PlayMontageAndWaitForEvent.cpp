...
void UAT_PlayMontageAndWaitForEvent::Activate()
{
	...
	UBaseChar_AbilitySystemComponent* GSAbilitySystemComponent = GetTargetASC();
	...
	if (GSAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = GSAbilitySystemComponent->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UAT_PlayMontageAndWaitForEvent::OnGameplayEvent));
...
UBaseChar_AbilitySystemComponent * UAT_PlayMontageAndWaitForEvent::GetTargetASC()
{
	return Cast<UBaseChar_AbilitySystemComponent>(AbilitySystemComponent);;
}
